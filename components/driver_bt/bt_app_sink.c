...
static void bt_toggle(bool pressed) {
  ...
  if (s_audio == AUDIO_PLAYING)
    ...
    esp_avrc_ct_send_passthrough_cmd(tl++, ESP_AVRC_PT_CMD_PLAY,
                                     ESP_AVRC_PT_CMD_STATE_PRESSED);
}
...
static void bt_play(bool pressed) {
  if (!pressed)
    return;
  esp_avrc_ct_send_passthrough_cmd(tl++ & 0x0f, ESP_AVRC_PT_CMD_PLAY,
                                   ESP_AVRC_PT_CMD_STATE_PRESSED);
}
...
static void bt_pause(bool pressed) {
  if (!pressed)
    return;
  esp_avrc_ct_send_passthrough_cmd(tl++ & 0x0f, ESP_AVRC_PT_CMD_PAUSE,
                                   ESP_AVRC_PT_CMD_STATE_PRESSED);
}
...
static void bt_prev(bool pressed) {
  if (!pressed)
    return;
  esp_avrc_ct_send_passthrough_cmd(tl++ & 0x0f, ESP_AVRC_PT_CMD_BACKWARD,
                                   ESP_AVRC_PT_CMD_STATE_PRESSED);
}
...
static void bt_next(bool pressed) {
  if (!pressed)
    return;
  esp_avrc_ct_send_passthrough_cmd(tl++ & 0x0f, ESP_AVRC_PT_CMD_FORWARD,
                                   ESP_AVRC_PT_CMD_STATE_PRESSED);
}
...
const static actrls_t controls = {
    ...
    NULL,           NULL,           NULL,     NULL, NULL, // pre1-10
    bt_volume_down, bt_volume_up,   bt_toggle // knob left, knob_right, knob
                                              // push
};
...
void update_metadata(bool force) {
  if ((s_metadata.updated || force) && s_audio == AUDIO_PLAYING) {
    ...
    (*bt_app_a2d_cmd_cb)(BT_SINK_METADATA, s_metadata.artist, s_metadata.album,
                         s_metadata.title);
...
static bool cmd_handler(bt_sink_cmd_t cmd, ...) {
  ...
  case BT_SINK_METADATA: {
    char *artist = va_arg(args, char *), *album = va_arg(args, char *),
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param) {
  ...
  case ESP_A2D_CONNECTION_STATE_EVT: {
    ...
    ESP_LOGD(BT_AV_TAG,
             ...
             s_a2d_conn_state_str[a2d->conn_stat.state], bda[0], bda[1], bda[2],
             bda[3], bda[4], bda[5]);
  ...
  case ESP_A2D_AUDIO_STATE_EVT: {
    ...
    if (ESP_A2D_AUDIO_STATE_STARTED == a2d->audio_stat.state) {
      ...
      if ((*bt_app_a2d_cmd_cb)(BT_SINK_AUDIO_STARTED, s_sample_rate)) {
        ...
        // if decoder is busy, stop it (would be better to not ACK this command,
        // but don't know how)
        esp_avrc_ct_send_passthrough_cmd(tl++ & 0x0f, ESP_AVRC_PT_CMD_STOP,
                                         ESP_AVRC_PT_CMD_STATE_PRESSED);
      }
  ...
  case ESP_A2D_AUDIO_CFG_EVT: {
    a2d = (esp_a2d_cb_param_t *)(p_param);
    ESP_LOGD(BT_AV_TAG, "A2DP audio stream configuration, codec type %d",
...
static void bt_av_notify_evt_handler(uint8_t event_id,
  ...
  case ESP_AVRC_RN_PLAY_STATUS_CHANGE:
    ESP_LOGD(BT_AV_TAG, "Playback status changed: 0x%x",
             event_parameter->playback);
    if (s_audio != AUDIO_IDLE) {
      switch (event_parameter->playback) {
      case ESP_AVRC_PLAYBACK_PLAYING:
        // if decoder is busy then stop (would be better to not ACK this
        // command, but don't know how)
      ...
      ESP_LOGW(BT_AV_TAG, "Not yet in BT connected mode: 0x%x",
               event_parameter->playback);
    }
    bt_av_playback_changed();
    break;
  case ESP_AVRC_RN_PLAY_POS_CHANGED:
    ESP_LOGD(BT_AV_TAG, "Play position changed: %d (ms)",
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param) {
  ...
  case ESP_AVRC_CT_PASSTHROUGH_RSP_EVT: {
    ESP_LOGD(BT_RC_CT_TAG, "AVRC passthrough rsp: key_code 0x%x, key_state %d",
             rc->psth_rsp.key_code, rc->psth_rsp.key_state);
    break;
  }
  case ESP_AVRC_CT_METADATA_RSP_EVT: {
    ESP_LOGD(BT_RC_CT_TAG, "AVRC metadata rsp: attribute id 0x%x, %s",
  ...
  case ESP_AVRC_CT_CHANGE_NOTIFY_EVT: {
    ESP_LOGD(BT_RC_CT_TAG, "AVRC event notification: %d",
  ...
  case ESP_AVRC_CT_REMOTE_FEATURES_EVT: {
    ESP_LOGD(BT_RC_CT_TAG, "AVRC remote features %x, TG features %x",
             rc->rmt_feats.feat_mask, rc->rmt_feats.tg_feat_flag);
    break;
  }
  case ESP_AVRC_CT_GET_RN_CAPABILITIES_RSP_EVT: {
    ESP_LOGD(BT_RC_CT_TAG, "remote rn_cap: count %d, bitmask 0x%x",
...
static void bt_av_hdl_avrc_tg_evt(uint16_t event, void *p_param) {
  ...
  case ESP_AVRC_TG_PASSTHROUGH_CMD_EVT: {
    ESP_LOGD(BT_RC_TG_TAG, "AVRC passthrough cmd: key_code 0x%x, key_state %d",
             rc->psth_cmd.key_code, rc->psth_cmd.key_state);
    break;
  }
  case ESP_AVRC_TG_SET_ABSOLUTE_VOLUME_CMD_EVT: {
    ESP_LOGD(BT_RC_TG_TAG, "AVRC set absolute volume: %d%%",
             (rc->set_abs_vol.volume * 100) / 127);
    volume_set_by_controller(rc->set_abs_vol.volume);
    break;
  }
  case ESP_AVRC_TG_REGISTER_NOTIFICATION_EVT: {
    ESP_LOGD(BT_RC_TG_TAG, "AVRC register event notification: %d, param: 0x%x",
  ...
  case ESP_AVRC_TG_REMOTE_FEATURES_EVT: {
    ESP_LOGD(BT_RC_TG_TAG, "AVRC remote features %x, CT features %x",
...
void bt_sink_init(bt_cmd_vcb_t cmd_cb, bt_data_cb_t data_cb) {
  ...
  char *item =
      config_alloc_get_default(NVS_TYPE_STR, "bt_sink_volume", "127", 0);
  ...
  char *pin_code = config_alloc_get_default(NVS_TYPE_STR, "bt_sink_pin",
                                            STR(CONFIG_BT_SINK_PIN), 0);
...
static void bt_av_hdl_stack_evt(uint16_t event, void *p_param) {
  ...
  case BT_APP_EVT_STACK_UP: {
    /* set up device name */
    bt_name = (char *)config_alloc_get_default(NVS_TYPE_STR, "bt_name",
                                               CONFIG_BT_NAME, 0);
