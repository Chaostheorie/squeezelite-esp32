...
_(opinions presented here so I = @philippe44)_
...
- spi*config: `"mosi=15,miso=2,clk=14` *(this one is probably optional)\_
...
- dac_controlset:
  ```json
  {
    ...
    "poweron": [{ "reg": 18, "val": 64, "mode": "or" }],
    "poweroff": [{ "reg": 18, "val": 191, "mode": "and" }]
  }
  ```
...
- rotate: for non-square _drivers_, move to portrait mode. Note that _width_ and _height_ must be inverted then
...
The SW gpio is optional, you can re-affect it to a pure button if you prefer but the volume, longpress and knobonly options make little sense as the missing switch plays an important role in these modes. You could still have the "volume" mode, but you won't be able to use it for _anything_ expect volume up and down. So be aware that the use of syntax [] is a bit misleading hereabove.
...
**Be aware that when using non "raw" mode, the CLI (Command Line Interface) of LMS is used and _must_ be available without password**
...
Be mindful that if the same GPIO is used to go to sleep and wakeup with the _same_ level (in other word it's a transition/edge that triggers the action) the above will not work and the esp32 will immediately restart. In such case, you case use a button definition. The benefit of buttons is that not only can you re-use one actual button (e.g. 'stop') to make it the sleep trigger (using a long-press or a shift-press) but by selecting the ACTRLS*SLEEP action upon 'release', you can got to sleep upon release (1-0-1 transition) but also wake up upon another press (0 level applied on GPIO) because you only go to sleep \_after* the GPIO returned to 1.
